...
- **csv 的數值欄是逐欄推論的**，不是逐格：一欄裡非空的格子全都是數字才算數值欄，所以文字欄裡夾一個 `42` 不會單獨跳色。開頭是 0 的（`007`、`0912345678`）當代碼不當數量，`0.5` 仍是數字。日期與布林值不上色——要上就得再給顏色，那就變成多色配色了
- **只有一個強調色。** 螢光綠 `--neon` 標出「值」：json 的純量、csv 的數值欄、yaml 冒號後面的部分。key 與標點保持安靜。一個顏色是強調，一整套 token 配色才是語法高亮，後者屬於 SnapDeck
